...
describe('GET /api/admin/export/products', () => {
  ...
  it('includes product data rows', async () => {
    await createTestProduct({ name: 'Export Test Saree B20' });
    const res = await request(app).get('/api/admin/export/products');
    expect(res.status).toBe(200);
    expect(res.text).toContain('Export Test Saree B20');
  });
...
describe('GET /api/admin/export/customers', () => {
  ...
  it('includes customer data when orders exist', async () => {
    const product = await createTestProduct({ name: 'CSV Customer Product B20' });
...
describe('GET /api/admin/export/orders', () => {
  ...
  it('includes order data rows', async () => {
    const product = await createTestProduct({ name: 'Export Order Product B20' });
...
describe('PUT /api/admin/orders/:id/status — quick mark shipped', () => {
  it('marks a placed order as shipped', async () => {
    const product = await createTestProduct({ name: 'Quick Ship Product B20' });
  ...
  it('marks a confirmed order as shipped', async () => {
    const product = await createTestProduct({ name: 'Confirmed Ship B20' });
  ...
  it('marks a packed order as shipped', async () => {
    const product = await createTestProduct({ name: 'Packed Ship B20' });
  ...
  it('can include trackingId when marking shipped', async () => {
    const product = await createTestProduct({ name: 'Tracked Ship B20' });
...
describe('GET /api/products — occasion filter', () => {
  it('filters by wedding occasion', async () => {
    await createTestProduct({ name: 'Bridal Lehenga B20', occasion: ['wedding'] });
...
describe('POST /api/inventory/back-in-stock', () => {
  it('registers email for back-in-stock notification', async () => {
    const product = await createTestProduct({ name: 'OOS Product B20', stock: 0 });
    const res = await request(app)
  ...
  it('registers phone for back-in-stock notification', async () => {
    const product = await createTestProduct({ name: 'OOS Product Phone B20', stock: 0 });
    const res = await request(app)
      .post('/api/inventory/back-in-stock')
  ...
  it('rejects request without email or phone', async () => {
    const product = await createTestProduct({ name: 'OOS No Contact B20', stock: 0 });
    const res = await request(app)
      .post('/api/inventory/back-in-stock')
...
describe('GET /api/admin/dashboard — polling readiness', () => {
  ...
  it('todayOrders increments after new order', async () => {
    ...
    const product = await createTestProduct({ name: 'Poll Test B20' });
  ...
  it('pendingOrders increments after new placed order', async () => {
    ...
    const product = await createTestProduct({ name: 'Pending Poll B20' });
  ...
  it('pendingOrders decreases after marking shipped', async () => {
    const product = await createTestProduct({ name: 'Ship Poll B20' });
...
describe('POST /api/admin/orders/bulk-status', () => {
  it('bulk-marks multiple orders as shipped', async () => {
    const product = await createTestProduct({ name: 'Bulk Ship B20' });
  ...
  it('bulk-marks single order as delivered', async () => {
    const product = await createTestProduct({ name: 'Single Bulk B20' });
...
describe('POST /api/coupons/validate — edge cases', () => {
  it('applies percent coupon correctly at minimum order', async () => {
    ...
    const res = await request(app)
      ...
      .send({ code: 'B20_EDGE10', total: 500 });
    expect(res.status).toBe(200);
  ...
  it('rejects coupon below minimum order', async () => {
    ...
    const res = await request(app)
      .post('/api/coupons/validate')
      .send({ code: 'B20_MIN500', total: 499 });
    expect(res.status).toBe(200);
  ...
  it('rejects expired coupon', async () => {
    ...
    const res = await request(app)
      .post('/api/coupons/validate')
      .send({ code: 'B20_EXPIRED', total: 1000 });
    expect(res.status).toBe(200);
    expect(res.body.valid).toBe(false);
  });
  ...
  it('rejects inactive coupon', async () => {
    ...
    const res = await request(app)
      .post('/api/coupons/validate')
      .send({ code: 'B20_INACTIVE', total: 1000 });
    expect(res.status).toBe(200);
    expect(res.body.valid).toBe(false);
  });
  ...
  it('handles flat type coupon', async () => {
    ...
    const res = await request(app)
      .post('/api/coupons/validate')
      .send({ code: 'B20_FLAT100', total: 800 });
    expect(res.status).toBe(200);
    expect(res.body.valid).toBe(true);
  });
  ...
  it('rejects unknown coupon code', async () => {
    const res = await request(app)
      .post('/api/coupons/validate')
      .send({ code: 'TOTALLY_FAKE_CODE_XYZ', total: 1000 });
    expect(res.status).toBe(200);
    expect(res.body.valid).toBe(false);
  });
});
...
describe('PATCH /api/admin/products/:id/stock', () => {
  it('updates product stock to zero (marks out-of-stock)', async () => {
    const product = await createTestProduct({ name: 'Zero Stock B20', stock: 10 });
    const res = await request(app)
  ...
  it('restores product stock from zero', async () => {
    const product = await createTestProduct({ name: 'Restock B20', stock: 0 });
    const res = await request(app)
      .patch(`/api/admin/products/${product.id}/stock`)
      .send({ stock: 50 });
    expect(res.status).toBe(200);
  ...
  it('updates stock to a large number', async () => {
    const product = await createTestProduct({ name: 'Big Stock B20', stock: 5 });
    const res = await request(app)
      .patch(`/api/admin/products/${product.id}/stock`)
      .send({ stock: 1000 });
    expect(res.status).toBe(200);
...
describe('GET /api/products — category + occasion combined', () => {
  it('returns products by category slug', async () => {
    const cat = await createTestCategory('Sarees B20');
    await createTestProduct({ name: 'Silk Saree B20', categoryId: cat.id });
    const res = await request(app).get(`/api/products?category=sarees-b20`);
    expect(res.status).toBe(200);
  });
  ...
  it('returns featured products only', async () => {
    await createTestProduct({ name: 'Featured B20', featured: true });
  ...
  it('returns best seller products only', async () => {
    await createTestProduct({ name: 'BestSeller B20', bestSeller: true });
...
describe('Order lifecycle — placed → confirmed → packed → shipped → delivered', () => {
  it('transitions through all statuses', async () => {
    const product = await createTestProduct({ name: 'Lifecycle Product B20', stock: 10 });
  ...
  it('allows cancellation from placed status', async () => {
    const product = await createTestProduct({ name: 'Cancel B20' });
  ...
  it('GET /api/order/:id returns full order details', async () => {
    const product = await createTestProduct({ name: 'View Order B20' });
  ...
  it('GET /api/orders/:id includes items array', async () => {
    const product = await createTestProduct({ name: 'Items Order B20' });
...
describe('GET /api/admin/orders — status filtering', () => {
  it('filters by placed status', async () => {
    const product = await createTestProduct({ name: 'Filter Placed B20' });
  ...
  it('filters by shipped status', async () => {
    const product = await createTestProduct({ name: 'Filter Shipped B20' });
  ...
  it('returns all orders without status filter', async () => {
    const product = await createTestProduct({ name: 'All Orders B20' });
...
describe('GET /api/products/search or GET /api/search', () => {
  it('finds product by name keyword', async () => {
    await createTestProduct({ name: 'Embroidered Anarkali B20' });
    const res = await request(app).get('/api/search?q=Anarkali+B20');
    if (res.status === 200) {
      const items = res.body.products || res.body;
      expect(Array.isArray(items)).toBe(true);
    } else {
      // Search endpoint might not exist at this path, skip gracefully
...
describe('GET /api/admin/customers', () => {
  ...
  it('includes customer with orders', async () => {
    const product = await createTestProduct({ name: 'Customer List B20' });
  ...
  it('supports search by phone', async () => {
    const product = await createTestProduct({ name: 'Search Customer B20' });
...
describe('GET /api/admin/low-stock', () => {
  it('returns products with stock at or below threshold', async () => {
    await createTestProduct({ name: 'Low Stock B20', stock: 3 });
    const res = await request(app).get('/api/admin/low-stock?threshold=5');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.products)).toBe(true);
    const found = res.body.products.find((p: { name: string }) => p.name === 'Low Stock B20');
    expect(found).toBeTruthy();
  });

  it('does not include products above threshold', async () => {
    await createTestProduct({ name: 'High Stock B20', stock: 100 });
    const res = await request(app).get('/api/admin/low-stock?threshold=5');
    expect(res.status).toBe(200);
    const found = res.body.products.find((p: { name: string }) => p.name === 'High Stock B20');
