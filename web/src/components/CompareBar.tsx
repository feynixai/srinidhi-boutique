...
export function CompareBar() {
  ...
  return (
    <>
      {/* Mobile: slim compact bar above bottom nav */}
      <div className="fixed bottom-[76px] left-1/2 -translate-x-1/2 z-50 animate-fade-in md:hidden">
        <div className="bg-[#1a1a2e]/95 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl px-4 py-2.5 flex items-center gap-2.5">
          <FiBarChart2 size={14} className="text-[#c5a55a] flex-shrink-0" />
          <Link
            href="/compare"
            className="bg-[#c5a55a] text-[#1a1a2e] text-xs font-semibold px-4 py-1.5 rounded-full hover:bg-[#d4b46a] transition-colors whitespace-nowrap"
          >
            {t.compareProducts} ({items.length})
          </Link>
          <button
            onClick={clear}
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-fade-in hidden md:block">
